...
#        self.classes_ = np.array(sorted(list(set(yl))))
    ...
    def real_boost(self, X, y, sample_weight):
        ...
        if len(self.estimators_) == 0:
            #Copy NN to estimator:
            estimator = self.deepcopy_NN(self.base_estimator_)#deepcopy of self.base_estimator_
        else: 
            #estimator = deepcopy(self.estimators_[-1])
            estimator = self.deepcopy_NN(self.estimators_[-1])#deepcopy NN
    ###################################################
...
#########################################################        
    ...
    def deepcopy_NN(self, base_estimator0):
        #Copy NN (self.base_estimator_) to estimator:
